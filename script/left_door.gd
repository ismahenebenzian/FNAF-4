...
@onready var dark_overlay = $MainInterface/Zoom_Porte_gauche/DarkOverlay
@onready var door_sprite = $MainInterface/Zoom_Porte_gauche/DoorSprite
@onready var fond_couloir = $MainInterface/Zoom_Porte_gauche/FondCouloir
@onready var son_souffle = $SonSouffle

var tex_normal = preload("res://assets/images/Zoom_Porte_Gauche.png")
var tex_loin   = preload("res://assets/images/monstre_couloir_gauche_loin.png")
...
func _input(event):
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed and not door_is_closed:
			...
			if MonstreManager.etat == MonstreManager.Etat.COULOIR_GAUCHE_DEVANT:
...
func _update_visuel_monstre():
	if MonstreManager.etat == MonstreManager.Etat.COULOIR_GAUCHE_LOIN:
		fond_couloir.texture = tex_loin
	else:
		fond_couloir.texture = tex_normal

func _update_son_souffle():
	var devant = MonstreManager.etat == MonstreManager.Etat.COULOIR_GAUCHE_DEVANT
